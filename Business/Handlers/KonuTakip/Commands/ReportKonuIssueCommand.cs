...
using Business.Handlers.KonuTakip.ValidationRules;
...
namespace Business.Handlers.KonuTakip.Commands
{
    /// <summary>Konu takip: eksik/yanlış konu bildirimi — gönderen yapılandırmadaki no-reply, alıcı support@masavtech.com.</summary>
    public class ReportKonuIssueCommand : IRequest<IResult>
    {
        ...
        public string SinavBolumAd { get; set; }
    }

    public class ReportKonuIssueCommandHandler : IRequestHandler<ReportKonuIssueCommand, IResult>
    {
        ...
        public ReportKonuIssueCommandHandler(IMailService mailService, IConfiguration configuration)
        {
            _mailService = mailService;
            _configuration = configuration;
        }

        [ValidationAspect(typeof(ReportKonuIssueValidator), Priority = 1)]
        [LogAspect(typeof(FileLogger))]
        public Task<IResult> Handle(ReportKonuIssueCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var safeSinav = WebUtility.HtmlEncode((request.SinavAd ?? string.Empty).Trim());
                var safeBolum = WebUtility.HtmlEncode((request.SinavBolumAd ?? string.Empty).Trim());
                var bodyText = request.Message ?? string.Empty;
                var safeBody = WebUtility.HtmlEncode(bodyText).Replace("\n", "<br>", StringComparison.Ordinal);

                var sinavRow = string.IsNullOrWhiteSpace(safeSinav)
                    ? string.Empty
                    : $"<strong>Sınav:</strong> {safeSinav}<br>";
                var bolumRow = string.IsNullOrWhiteSpace(safeBolum)
                    ? string.Empty
                    : $"<strong>Bölüm:</strong> {safeBolum}<br>";
...
<body>
    <div class='container'>
        <div class='header'>
            <h2 style='margin:0;font-size:20px'>Konu Takip Bildirimi</h2>
        </div>
        <div class='content'>
            <div class='app-info'>
            ...
            <div class='info'>
                ...
                <strong>Bildirim Türü:</strong> {safeSubject}<br>
                {sinavRow}
                {bolumRow}
            </div>
            <div class='message'>
                ...
                var emailMessage = new EmailMessage
                {
                    ...
                    Subject = $"{appName} - Konu Bildirimi: {request.Subject.Trim()}",
                ...
                return Task.FromResult<IResult>(new SuccessResult("Bildiriminiz başarıyla gönderildi."));
            }
            catch (Exception ex)
            {
                return Task.FromResult<IResult>(
                    new ErrorResult("Bildirim gönderilirken bir hata oluştu: " + ex.Message));
            }
        }
    }
}
